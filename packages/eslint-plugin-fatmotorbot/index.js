...
module.exports = {
  rules: {
    "max-lines-per-function-ignore-nested": {
      meta: {
        type: "suggestion",
      ...
      create(context) {
        ...
        function isNested(node) {
          return node.type === "FunctionDeclaration" && node.parent != null && node.parent.type === "FunctionDeclaration";
        }
        ...
        function processFunction2(node) {
          if (node.parent.type === 'BlockStatement') {
            // nodes.push([node.loc.start.line, node.loc.end.line]);
        ...
        function shouldIgnore(parentType, i) {
          return parentType !== 'BlockStatement' && nodesSet.has(i);
        ...
        function processFunction(funcNode) {
          ...
          // console.log(astUtils.getFunctionNameWithKind(funcNode), funcNode.parent.type);
          ...
          for (let i = node.loc.start.line - 1; i < node.loc.end.line; ++i) {
            ...
            if (shouldIgnore(node.parent.type, i)) continue;
          ...
          if (lineCount > maxLines) {
            const name = getFunctionNameWithKind(funcNode);
        ...
        return {
          FunctionDeclaration: processFunction2,
          'FunctionDeclaration:exit': processFunction,
