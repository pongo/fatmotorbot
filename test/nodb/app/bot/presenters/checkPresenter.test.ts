...
`.trim()
    ...
    assert.strictEqual(
      present(checkUseCase(['ж', '164', '48'])),
      `
Женщина, 164 см, 48 кг.

ИМТ: 18.12 — у тебя неопасный дефицит веса. Тебе нужно набрать 3 кг до здорового веса, который для тебя от 50.34 до 63.56 кг. А твой идеальный вес: 57 кг (53–64).
`.trim()
    );
  });
});
