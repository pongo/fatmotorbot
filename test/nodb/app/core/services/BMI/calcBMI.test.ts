...
import { calcBMI, GetBMIUseCase } from 'src/app/core/services/BMI/BMI';
...
describe('calcBMI', () => {
  ...
  it('no user info', async () => {
    ...
    const usecase = new GetBMIUseCase(repo, weightRepo);
  ...
  it('return bmi data', async () => {
    ...
    const weightRepo = WeightRepositoryMockSinon();
    const usecase = new GetBMIUseCase(infoRepository, weightRepo);
